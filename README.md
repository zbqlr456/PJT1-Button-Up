...
- 익숙한 **MyBatis**가 아니라 **JPA**를 선택한 이유
    - 소규모 프로젝트 + 복잡한 쿼리가 쓰이지 않을 것이라고 생각했기 때문에
    - 객체지향적으로 데이터를 관리한다는 것을 직접 체험해보고 싶었기 때문에
    - 그리고 JPA의 단점(복잡한 쿼리는 짜기 힘들다, 복잡한 내부로 인한 성능 이슈) 등은 다른 방법들을 통하면 단점보다 더 많은 이점을 얻을 수 있기 때문에 현재 단계에선 고려사항이 아니라고 생각
